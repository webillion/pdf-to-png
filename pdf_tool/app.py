...
def process_pdf_to_images(pdf_path, dpi, is_transparent):
    try:
        ...
        # 複数ファイル時は1ファイルあたり15ページまでに制限してメモリを保護
        pages = pages[:15] 
        output_images = []
...
def index():
    if request.method == 'POST':
        ...
        zip_buffer.seek(0)
        response = make_response(send_file(zip_buffer, mimetype='application/zip', as_attachment=True, download_name="converted_materials.zip"))
        response.set_cookie('download_started', 'true', path='/')
        return response
